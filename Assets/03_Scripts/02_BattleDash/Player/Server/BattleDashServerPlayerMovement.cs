...
{
	public class BattleDashServerPlayerMovement : NetworkBehaviour
	{
		...
		{
			BattleDashServerPlayerInputEvents.PlayerInputKeyDown += OnPlayerInputKeyDown;
			BattleDashServerPlayerInputEvents.PlayerInputKeyUp += OnPlayerInputKeyUp;
			BattleDashServerPlayerInputEvents.PlayerMobileTouchPosition += OnPlayerMobileTouchPosition;
		}
		...
		{
			LoggerService.LogInfo($"{nameof(BattleDashServerPlayerMovement)}::{nameof(OnPlayerMobileTouchPosition)}- press: {worldTouchPos}");
			...
			LoggerService.LogInfo($"{nameof(BattleDashServerPlayerMovement)}::{nameof(OnPlayerMobileTouchPosition)}- current movement: {_movement}");
		}

		private void OnPlayerInputKeyDown(KeyCode keyCode)
		{
			LoggerService.LogInfo($"{nameof(BattleDashServerPlayerMovement)}::{nameof(OnPlayerInputKeyDown)}- press: {keyCode}");
			_directionChangeTimer = 0;
			switch (keyCode){
				case KeyCode.A:
					_isAPressed = true;
			...
			LoggerService.LogInfo($"{nameof(BattleDashServerPlayerMovement)}::{nameof(OnPlayerInputKeyDown)}- current movement: {_movement}");
		}

		private void OnPlayerInputKeyUp(KeyCode keyCode)
		{
			LoggerService.LogInfo($"{nameof(BattleDashServerPlayerMovement)}::{nameof(OnPlayerInputKeyUp)}- press: {keyCode}");
			_directionChangeTimer = 0;
			switch (keyCode){
				case KeyCode.A:
					_isAPressed = false;
			...
			LoggerService.LogInfo($"{nameof(BattleDashServerPlayerMovement)}::{nameof(OnPlayerInputKeyUp)}- current movement: {_movement}");
		}
	    ...
	    private void Update()
	    {
		    BattleDashClientActionEvents.RaiseUpdatePlayerVisualPositionEvent(this.transform.position);
	    }
#endif
	}
}