...
namespace PeanutDashboard.Server
{
	public static class ServerService
	{
		...
		private static UnityWebRequest SetupPostWebRequest(string api, string formData)
		{
			...
			string encryptedFormData = EnvironmentManager.Instance.UseRSA ? Encrypt(formData) : formData;
			string serverUrl = EnvironmentManager.Instance.ServerUrl;
			...
			if (!string.IsNullOrEmpty(UserService.TonToken))
			{
				webRequest.SetRequestHeader("Authorization", "Bearer " + UserService.TonToken);
			}
